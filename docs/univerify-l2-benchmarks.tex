...
\begin{table}[H]
  \centering
  \caption{Tabela 1a: Zużycie gazu \texttt{issueBatch} według sieci i rozmiaru paczki. Wartości deterministyczne (pojedyncza próbka per (sieć, rozmiar)).}
...
\begin{table}[H]
  \centering
  \caption{Tabela 1b: Koszt \texttt{issueBatch} w USD (scenariusz p50, $\text{ETH}/\text{USD}{=}3500$). Pełna zmienność scenariuszy p10/p50/p90 dla $n{=}1000$ na Rys.~4.}
  \pgfplotstabletypeset[
    col sep=comma,
    string type,
    every head row/.style={before row=\toprule, after row=\midrule},
...
\begin{table}[H]
  \centering
  \caption{Tabela 5: Czas odbioru paragonu transakcji zapisu po stronie klienta (ms). Kolumny \texttt{issue} pochodzą z $N{=}30$ powtórzeń \texttt{issueBatch(1000)} w każdym runie i są agregowane przez wszystkie trzy niezależne runy.}
  \pgfplotstabletypeset[
    col sep=comma,
    string type,
    every head row/.style={before row=\toprule, after row=\midrule},
...
\section{Wyniki: analizy wrażliwości}

\subsection{Scenariusze basefee L1}
\begin{figure}[H]
  \centering
  \begin{tikzpicture}
    \begin{axis}[
...
Koszt w USD jest liniową funkcją kursu ETH/USD: $C(r) = C_{3500} \cdot r/3500$, gdzie $C_{3500}$ to wartość bazowa zaraportowana przy snapshocie z dnia 2026-05-13 ($r=3500$). Tabela~6 pokazuje koszt \texttt{issueBatch(1000)} przy scenariuszu basefee p50 dla trzech zakotwiczonych poziomów rynkowych.

\begin{table}[H]
  \centering
  \caption{Tabela 6: Wrażliwość kosztu \texttt{issueBatch(1000)} na kurs ETH/USD (basefee p50).}
  \csvautotabular{\DataDir/table-sensitivity-eth-usd.csv}
\end{table}

\subsection{Wrażliwość na ceny gazu sekwensera L2}
...
\]
gdzie $C_{\text{exec}} = \texttt{gasUsed} \cdot p_{\text{L2}}$ skaluje się liniowo z ceną sekwensera $p_{\text{L2}}$, natomiast $C_{\text{L1 data}}$ pochodzi z blob basefee na L1 i pozostaje stała. Tabela~7 pokazuje koszt \texttt{issueBatch(1000)} przy trzech mnożnikach względem snapshotu cen w \texttt{packages/benchmarks/src/config.ts} (Arbitrum 0.1\,gwei, Base 0.005\,gwei, zkSync 0.05\,gwei). Sepolia pominięta --- jako sieć L1 nie ma ceny sekwensera.

\begin{table}[H]
  \centering
  \caption{Tabela 7: Wrażliwość kosztu \texttt{issueBatch(1000)} na cenę gazu sekwensera L2 (ETH/USD=3500, basefee p50; mnożniki względem snapshotu z \texttt{config.ts}).}
  \csvautotabular{\DataDir/table-sensitivity-l2-price.csv}
\end{table}

\section{Dyskusja}

{\sloppy
Wszystkie pomiary opisane w niniejszym rozdziale wykonano przez jednego dostawcę RPC w jednej lokalizacji POP, z $N{=}30$ powtórzeniami dla każdego pomiaru opóźnienia oraz losowym wyborem liści przy revoke. Wyniki to średnia $\pm\sigma$ z trzech niezależnych runów. Wszystkie parametry konfiguracyjne (ziarno losowości, dostawca, region, kurs ETH/USD) są zapisane w pliku \texttt{\DataDir/meta.json}.\par}
