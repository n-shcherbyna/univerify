...
\begin{tikzpicture}[node distance=10mm and 20mm]
  ...
  \draw[strz] (ms) -- node[etyk, above=1mm] {\code{schedule}} (tl);
  \draw[strz] (tl) -- node[etyk, above=1mm] {\code{execute}\\po opóźnieniu} (reg);
  \draw[strz, dashed] (ms.south) to[out=-30, in=-150]
    node[etyk, below=1.5mm] {\code{cancel} w~oknie opóźnienia} (tl.south);
  \node[etyk, below=17mm of tl]
    {zdarzenie \code{CallScheduled} --- zmiana publicznie widoczna, zanim nabierze mocy};
\end{tikzpicture}
...
rejestracji pojedynczego dyplomu zmierzony w~rozdziale~\ref{ch:l2} --- od
$9\cdot10^{-7}$~USD w~najtańszej badanej sieci warstwy drugiej --- który
...
Pozostaje pytanie o~proporcję. Ta jedna własność kosztuje od $9\cdot10^{-7}$ do
około $2\cdot10^{-5}$~USD na dyplom w~warstwie drugiej, zależnie od sieci
(rozdz.~\ref{ch:l2}), plus jednorazowy, amortyzowany narzut
