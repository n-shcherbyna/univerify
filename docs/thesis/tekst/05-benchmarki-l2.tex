...
\code{issueBatch(1000)} w~trzech jej scenariuszach (p10/p50/p90).
...
zaznaczono w~sekcji~\ref{sec:l2-latencja}, pomiar czasu od wysłania do paragonu
...
Wynik ten jest odporny na typową zmienność rynkową, choć nie wszystkie zmienne
działają tak samo (sekcja~\ref{sec:l2-wrazliwosc}). Skalowanie kursem ETH/USD jest
liniowe --- zachowuje zarówno uporządkowanie sieci, jak i~rząd wielkości kosztu ---
a~zmiana ceny sekwensera L2 również nie narusza kolejności rollupów. Wahania opłaty
bazowej L1 wpływają natomiast silnie na samą Sepolię, która jako sieć warstwy
pierwszej jest na nie bezpośrednio wystawiona: przy wyższej opłacie jej koszt rośnie,
przez co dystans rollupów do warstwy pierwszej jeszcze się powiększa. We wszystkich
scenariuszach Base pozostaje najtańsza, a~rollupy zachowują stabilny, niski koszt ---
z~perspektywy kosztowej Base jest zatem najkorzystniejszym celem wdrożenia rejestru
dyplomów spośród badanych sieci.
