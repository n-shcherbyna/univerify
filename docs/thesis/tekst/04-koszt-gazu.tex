...
koszt gazu. Zamiast oznaczać je literami, nadano im nazwy opisujące \emph{kluczową
właściwość} każdej wersji --- bo to właśnie ona odróżnia ją od poprzedniej.
...
Nie każda zmiana poprawia sytuację --- \code{TimestampRevoke} to klasyczny przykład
dodania czegoś z~dobrej intencji, co kosztuje dużo więcej niż zakładano:
przechowywanie timestampu rewokacji w~storage zamiast w~eventach zwiększyło koszt
\code{revoke} o~61\%. Sumarycznie, od \code{StringErrors} do \code{MinimalRecord}:
\[
...
jest właściwa metryka do porównania. Właściwą jest \textbf{koszt na dyplom}, bo
jedna transakcja \code{issueBatchRoot} obejmuje $N$ dyplomów:
\[
...
Skoro architektura Merkle jest już wybrana, pojawia się nowe pytanie: czy ścieżka
weryfikacji dowodu daje się jeszcze zoptymalizować na poziomie kodu EVM? Kontrakt
...
\code{revokeFromBatch} ($-1.4\%$ avg), bo stanowi mniejszą bazę kosztową (mniej
...
Zanim przejdę do liczb w~funkcji głębokości drzewa, warto zrozumieć, co naprawdę
...
\paragraph{Koszt testu vs koszt produkcyjny.}
...
architektury to inna liga}: model Merkle batch nie ,,poprawia'' istniejącego kodu
--- zadaje inne pytanie na poziomie projektu. Przy partii 100 dyplomów koszt
...
Merkle są przewidywalne i~liniowe}: wzór $\Delta(d) = 2^d \cdot 42 + 96 + d \cdot
199$ dokładnie opisuje zmierzone oszczędności --- wiemy \emph{skąd} pochodzi każdy
