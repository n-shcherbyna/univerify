...
/// @title DiplomaRegistryOZ — Merkle diploma registry using OpenZeppelin MerkleProof
/// @notice Identical behaviour to DiplomaRegistry (baseline); the only difference
///         is that Merkle proof verification uses OpenZeppelin's MerkleProof library
///         instead of a hand-rolled _processProof(). Leaf construction uses
///         abi.encodePacked (same as baseline, no assembly).
contract DiplomaRegistryOZ {
    ...
    function verifyBatchMembership(
        ...
        return MerkleProof.verifyCalldata(proof, root, merkleLeaf(docHash, batchId, issuer_));
    }
    ...
    function _statusWithProof(
        ...
        if (!MerkleProof.verifyCalldata(proof, root, leaf)) return Status.Unknown;
        return Status.Valid;
    }
