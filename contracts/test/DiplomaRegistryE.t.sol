...
import "../src/DiplomaRegistryE.sol";

contract DiplomaRegistryEGasTest is Test {
    DiplomaRegistryE reg;
    ...
    function testGasRevert_addIssuer_onlyOwner() public {
        vm.prank(attacker);
        vm.expectRevert(DiplomaRegistryE.OnlyOwner.selector);
        reg.addIssuer(address(0xCAFE));
    }
    ...
    function testGasRevert_issue_onlyIssuer() public {
        vm.prank(attacker);
        vm.expectRevert(DiplomaRegistryE.OnlyIssuer.selector);
        reg.issue(docHash);
    }
    ...
    function testGasRevert_issue_alreadyIssued() public {
        ...
        vm.prank(issuer1);
        vm.expectRevert(DiplomaRegistryE.AlreadyIssued.selector);
        reg.issue(docHash);
    }
    ...
    function testGasRevert_revoke_alreadyRevoked() public {
        ...
        vm.prank(issuer1);
        vm.expectRevert(DiplomaRegistryE.AlreadyRevoked.selector);
        reg.revoke(docHash);
    }
    ...
    function testGasRevert_revoke_notIssuerOfRecord() public {
        ...
        vm.prank(issuer2);
        vm.expectRevert(DiplomaRegistryE.NotIssuerOfRecord.selector);
        reg.revoke(docHash);
    }
