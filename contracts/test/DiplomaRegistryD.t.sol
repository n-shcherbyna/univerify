...
import "../src/DiplomaRegistryD.sol";

contract DiplomaRegistryDGasTest is Test {
    DiplomaRegistryD reg;
    ...
    function testGasRevert_addIssuer_onlyOwner() public {
        vm.prank(attacker);
        vm.expectRevert(DiplomaRegistryD.OnlyOwner.selector);
        reg.addIssuer(address(0xCAFE));
    }
    ...
    function testGasRevert_issue_onlyIssuer() public {
        vm.prank(attacker);
        vm.expectRevert(DiplomaRegistryD.OnlyIssuer.selector);
        reg.issue(docHash);
    }
    ...
    function testGasRevert_issue_alreadyIssued() public {
        ...
        vm.prank(issuer1);
        vm.expectRevert(DiplomaRegistryD.AlreadyIssued.selector);
        reg.issue(docHash);
    }
    ...
    function testGasRevert_revoke_alreadyRevoked() public {
        ...
        vm.prank(issuer1);
        vm.expectRevert(DiplomaRegistryD.AlreadyRevoked.selector);
        reg.revoke(docHash);
    }
    ...
    function testGasRevert_revoke_notIssuerOfRecord() public {
        ...
        vm.prank(issuer2);
        vm.expectRevert(DiplomaRegistryD.NotIssuerOfRecord.selector);
        reg.revoke(docHash);
    }
